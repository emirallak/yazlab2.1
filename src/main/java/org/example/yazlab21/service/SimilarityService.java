...
public class SimilarityService {
    ...
    private String normalizeText(String text) {
        ...
        return text.toLowerCase().replaceAll("[^a-z0-9ğüşiöç\\s]", " ").replaceAll("\\s+", " ").trim();
    }
    ...
    public String findMostSpecificLocation(List<String> locations) {
        ...
        return locations.stream().filter(loc -> loc != null && !loc.isEmpty()).max((loc1, loc2) ->
                {
                ...
                return Integer.compare(loc1.length(), loc2.length());}).orElse(null);
    }
